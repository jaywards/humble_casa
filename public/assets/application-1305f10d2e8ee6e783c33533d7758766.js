...
(function() {
  var nextScheduledMonthDay, nextScheduledWeekDay, populateForm, setDatePickers, showAndHideFields, validateForm, validateOneTimeRequest, validateRepeatingRequest;
  ...
  $(function() {
    if ($('body').hasClass("master_service_requests") && $(".sr-form").length > 0) {
      ...
      $(".onetime").change(function() {
        if ($(this).val() === "true") {
          return showAndHideFields('.onetime-fields', '.repeating-fields');
        } else {
          return showAndHideFields('.repeating-fields', '.onetime-fields');
        }
      });
      $(".frequency").change(function() {
        $('#first-scheduled-fields').hide(400);
        if ($(this).val() === "weekly" || $(this).val() === "every_other_week") {
          return showAndHideFields('.weekly-fields', '.monthly-fields');
        } else {
          return showAndHideFields('.monthly-fields', '.weekly-fields');
        }
      });
      $("#daypicker").change(function() {
        nextScheduledWeekDay($(this).val());
        return $('#first-scheduled-fields').show(400);
      });
  ...
  setDatePickers = function() {
    ...
    $('#start-date-picker').datetimepicker({
      ...
      onSelect: function(dateText, inst) {
        ...
        return $("#end-date-picker").datetimepicker("option", "minDate", endD);
      }
    });
    minD.setDate(minD.getDate() + 1);
    return $('#end-date-picker').datetimepicker({
      timeFormat: 'h:mmtt',
      stepMinute: 15,
  ...
  populateForm = function() {
    var e, selectedFrequency, stringDate;
    ...
    if ($("#master_service_request_onetime_false").attr("checked")) {
      ...
      return $('#first-scheduled-fields').show();
    }
  };

  showAndHideFields = function(showField, hideField) {
    $(showField).show(400);
    return $(hideField).hide(400);
  };

  nextScheduledWeekDay = function(day) {
    var dateString, days, scheduled_date, selected_day_num, today, today_day;
    ...
    return $(".service_end_date").val(scheduled_date);
  };

  nextScheduledMonthDay = function(selected_day_num) {
    var dateString, scheduled_date, today, today_day;
    today = new Date();
  ...
  validateForm = function() {
    if ($("#master_service_request_onetime_true").is(":checked")) {
      return validateOneTimeRequest();
    } else if ($("#master_service_request_onetime_false").is(":checked")) {
      return validateRepeatingRequest();
    } else {
  ...
  validateOneTimeRequest = function() {
    var enddate, startdate, today, xstartdate;
  ...
  validateRepeatingRequest = function() {
    var frequency;
  ...
  $(function() {
    var futureNextScheduledMonthDay, futureNextScheduledWeekDay, stringDate;
    if ($('body').hasClass("master_service_requests") && $(".pause-form").length > 0) {
      ...
      stringDate = new Date(document.getElementById("master_service_request_first_scheduled").value.substring(0, 19).replace(/-/g, "/")).toDateString();
      $('#date-string').text(stringDate);
      $(".paused").change(function() {
        var day, frequency;
        if ($(this).val() === "true") {
          return $(".master_service_request_restart_date").show(400);
        } else {
          $(".master_service_request_restart_date").hide(400);
          frequency = $("#master_service_request_frequency").val();
          if (frequency === "weekly" || frequency === "every_other_week") {
            day = $("#master_service_request_service_week_day").val();
            return nextScheduledWeekDay(day, $(this).val());
          } else {
            day = $("#master_service_request_service_month_day").val();
            return nextScheduledMonthDay(day, $(this).val());
          }
        }
      });
      $("#restart-date-picker").change(function() {
        var day, frequency;
        frequency = $("#master_service_request_frequency").val();
        if (frequency === "weekly" || frequency === "every_other_week") {
          day = $("#master_service_request_service_week_day").val();
          return futureNextScheduledWeekDay(day, $(this).val());
        } else {
          day = $("#master_service_request_service_month_day").val();
          return futureNextScheduledMonthDay(day, $(this).val());
        }
      });
    }
    futureNextScheduledWeekDay = function(day, future) {
      var dateString, days, futureDate, future_day, scheduled_date, selected_day_num;
      ...
      $('.first_scheduled').val(scheduled_date);
      dateString = scheduled_date.toDateString();
      return $('#date-string').text(dateString);
    };
    return futureNextScheduledMonthDay = function(selected_day_num, future) {
      var dateString, futureDate, future_day, scheduled_date;
      futureDate = new Date(future.substring(6), future.substring(0, 2) - 1, future.substring(3, 5));
...
(function() {
  var activate, deactivate, initializeListGroup, initializeRaty, property, validateForm;
  ...
  $(function() {
    if ($('body').hasClass("properties") && $(".property-form").length > 0) {
      ...
      return $("[id^=edit_property]").enableClientSideValidations();
    }
  });
  ...
  property = {
    setupPaymentForm: function() {
      return $('[id^=edit_property]').submit(function() {
    ...
    processCard: function() {
      ...
      return Stripe.createToken(card, property.handleStripeCardResponse);
    },
    handleStripeCardResponse: function(status, response) {
      if (status === 200) {
        ...
        return $('[id^=edit_property]')[0].submit();
      } else {
  ...
  $(function() {
    if ($('body').hasClass("properties") && $(".register-services").length) {
      initializeRaty();
      return initializeListGroup();
    }
  });

  initializeRaty = function() {
  ...
  initializeListGroup = function() {
    return $(".list-group-item").click(function(event) {
      var assignID, assignmentField, checkboxID, checkedField, currentServiceID, serviceID, serviceListing;
      serviceListing = $(this);
      ...
      if ($(event.target).hasClass("selected")) {
        if ($(checkedField).val() === 'true') {
          return deactivate(serviceListing, assignID, serviceID, checkedField);
        } else {
          return activate(serviceListing, assignID, serviceID, checkedField, currentServiceID);
        }
      } else {
  ...
  activate = function(serviceListing, assignID, serviceID, checkedField, currentServiceID) {
    var current;
    $('#selected' + currentServiceID + '-' + assignID).each(function() {
    ...
    $('#selected' + serviceID + '-' + assignID).each(function() {
    ...
    if (current === "false") {
      return $('#confirmed' + assignID).show(400);
    }
  };

  deactivate = function(serviceListing, assignID, serviceID, checkedField) {
    ...
    return $('#current_service' + assignID).val('nil');
  };

}).call(this);
(function() {
  var validateScheduling;
  ...
  $(function() {
    ...
    if ($('body').hasClass("service_requests") && $(".complete-request-form").length > 0) {
      ...
      return $('#completed-date-picker').datepicker({
    ...
    if ($('body').hasClass("service_requests") && $(".assign-request-form").length > 0) {
      ...
      $(".scheduled").change(function() {
        if ($(this).val() === "true") {
          ...
          scheduled = "false";
          return $('#service_request_first_scheduled').val(startD);
        }
      });
      return $("#save-btn").click(function() {
        if (scheduled === "true") {
...
(function() {
  ...
  $(function() {
    if ($('body').hasClass("services") && $(".service-form").length > 0) {
      ...
      return $("#service_phone").mask("(999) 999-9999");
    }
  });
  ...
  $(function() {
    if ($('body').hasClass("services") && $(".add-employee").length) {
      ...
      return $("#service_user_first_name").focus();
    }
  });
  ...
  $(function() {
    if ($('body').hasClass("services") && $(".service-payment-form").length > 0) {
      Stripe.setPublishableKey($('meta[name="stripe-key"]').attr('content'));
      $('#service_invoice_me').change(function() {
  ...
  service = {
    setupPaymentForm: function() {
      return $('[id^=edit_service]').submit(function() {
        $('input[type=submit]').attr('disabled', true);
    ...
    processCard: function() {
      ...
      return Stripe.createToken(card, service.handleStripeCardResponse);
    },
    handleStripeCardResponse: function(status, response) {
      if (status === 200) {
        ...
        return $('[id^=edit_service]')[0].submit();
      } else {
        $('#stripe_error').text(response.error.message);
        return $('input[type=submit]').attr('disabled', false);
      }
  ...
  $(function() {
    if ($('body').hasClass("services") && $(".provide-estimate").length > 0) {
      ...
      return $("#service_assignments_attributes_0_cost").focus();
    }
  });
...
(function() {
  $(function() {
    return $('#home-tabs > li > a').hover(function() {
  ...
  $(function() {
    if ($('body').hasClass("static_pages")) {
      $('#options_report_start_date').datepicker({
        dateFormat: 'MM-dd-yy'
      });
      return $('#options_report_end_date').datepicker({
        dateFormat: 'MM-dd-yy'
      });
    }
  });
  ...
  $(function() {
    var lastTab, reportLastTab;
    $("a[data-toggle=\"tab\"]").on("click", function(e) {
      if ($(e.target).hasClass('service-tab')) {
        return localStorage.setItem("lastTab", $(e.target).attr("href"));
      } else {
        return localStorage.setItem("reportLastTab", $(e.target).attr("href"));
      }
    });
    ...
    if (reportLastTab) {
      return $("a[href=\"" + reportLastTab + "\"]").click();
    }
  });
          ...
          return $.ajax({
            ...
            success: function(data) {
              ...
              if (data === "record_not_found") {
                return alert("Unable to process your rating at this time. Please try again later");
              } else {
                msgID = '.votemsg' + id;
                return $(msgID).text("Thank you for your feedback!");
              }
            }
          });
        }
...
(function() {
  $(function() {
    if ($('body').hasClass("users") && $(".user-form").length) {
      ...
      return $("#user_primary_phone").mask("(999) 999-9999");
    }
  });
  ...
  $(function() {
    ...
    if ($("body").hasClass("users") && $(".user-employer-select").length) {
      ...
      return $("#employer_state").change(function() {
        var services_options, state;
        state = $(this).val();
        services_options = '<select>';
        $.each(services, function(i, service) {
          if (service.state === state) {
            return services_options += '<option value="' + service.id + '">' + service.name + '</option>';
          }
        });
        services_options += '</select>';
        $("#user_employment_attributes_service_id").html(services_options);
        return $("#user_employment_attributes_service_id").attr('disabled', false);
      });
    }
  });
...
(function() {
  $(function() {
    return $(".launch-login-form").click(function() {
      return setTimeout((function() {
        $("#user_session_email").focus();
        return $("#login").enableClientSideValidations();
      }), 500);
    });
  });
